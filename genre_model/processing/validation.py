...
def drop_na_inputs(*, input_data: pd.DataFrame) -> pd.DataFrame:
    ...
    new_vars_with_na = [
        var
        for var in config.ml_model_config.features
        if var not in config.ml_model_config.categorical_vars_with_na + config.ml_model_config.numerical_vars_with_na
...
def validate_inputs(*, input_data: pd.DataFrame) -> Tuple[pd.DataFrame, Optional[dict]]:
    ...
    input_data.drop(labels=config.ml_model_config.variables_to_drop, axis=1, inplace=True)
