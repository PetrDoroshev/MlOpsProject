...
def make_prediction(
    *,
    ...
    preds = _price_pipe.predict(X=validated_data[config.ml_model_config.features])
    probs = _price_pipe.predict_proba(X=validated_data[config.ml_model_config.features])[:, 1]
